...
class ChannelSnippet : public client::JsonCppData {
 public:
  ...
  /**
   * Determine if the '<code>customUrl</code>' attribute was set.
   *
   * @return true if the '<code>customUrl</code>' attribute was set.
   */
  bool has_custom_url() const {
    return Storage().isMember("customUrl");
  }

  /**
   * Clears the '<code>customUrl</code>' attribute.
   */
  void clear_custom_url() {
    MutableStorage()->removeMember("customUrl");
  }


  /**
   * Get the value of the '<code>customUrl</code>' attribute.
   */
  const StringPiece get_custom_url() const {
    const Json::Value& v = Storage("customUrl");
    if (v == Json::Value::null) return StringPiece("");
    return StringPiece(v.asCString());
  }

  /**
   * Change the '<code>customUrl</code>' attribute.
   *
  ...
  void set_custom_url(const StringPiece& value) {
    *MutableStorage("customUrl") = value.data();
  }
